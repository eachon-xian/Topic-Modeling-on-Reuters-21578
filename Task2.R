...
dtm <- DocumentTermMatrix(pre_process_reuters)

#document term matrix TF-IDF
dtm1 <- DocumentTermMatrix(pre_process_reuters, control = list(weighting = weightTfIdf))

#To reduce the dimension of the DTM, we can remove the less frequent terms such that the sparsity is less than 0.95
review_dtm = removeSparseTerms(dtm1, 0.99)
